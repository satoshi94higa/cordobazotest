    ...
    <div className="relative w-screen h-screen bg-editorial-bg text-editorial-text overflow-hidden font-sans">
      ...
      <header className="absolute top-0 left-0 right-0 z-[1000] p-4 md:p-8 flex justify-between items-start pointer-events-none">
        ...
        <div className="flex gap-2 pointer-events-auto">
          <button 
            ...
            className="w-10 h-10 md:w-12 md:h-12 bg-editorial-bg border border-editorial-text/20 flex items-center justify-center hover:bg-editorial-text hover:text-white transition-all shadow-xl"
          >
            <HelpCircle size={18} />
          </button>
        </div>
      ...
      <AnimatePresence>
        {selectedPoint && (
          <motion.aside
            ...
            className="absolute top-0 left-0 h-full w-full md:w-[600px] bg-editorial-bg shadow-2xl z-[2000] border-r border-editorial-text/10 flex flex-col p-5 sm:p-8"
          >
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
              <PhotoSlider 
                ...
                hasNext={hasNext}
                hasPrev={hasPrev}
                className="mb-4 shadow-2xl border border-editorial-text/5"
              />
      ...
      <aside className="absolute top-32 right-8 bottom-24 w-52 z-[1000] hidden lg:flex flex-col gap-4 pointer-events-none">
        <div className="flex flex-col gap-0.5 pointer-events-auto">
          <div className="text-[9px] uppercase tracking-[0.4em] font-bold text-brand-primary mb-1 border-b border-editorial-text/10 pb-1 flex items-center gap-2">
          ...
          <div className="flex flex-col gap-0.5 overflow-y-auto max-h-[85vh] pr-1.5 custom-scrollbar">
            {CORDOBAZO_POINTS.sort((a, b) => a.order - b.order).map((point) => (
              <button
                ...
                className={`group flex items-start gap-2 p-2 text-left transition-all border ${
                  selectedPoint?.id === point.id 
                    ? "bg-editorial-text text-white border-editorial-text shadow-sm" 
                    : "bg-editorial-bg/80 backdrop-blur-md border-editorial-text/5 hover:border-editorial-text/20"
                }`}
              >
                ...
                <div className="flex-1">
                  <h4 className="text-[9px] font-bold uppercase tracking-widest leading-none">
                    {point.title}
                  </h4>
        ...
        {showMobilePoints && (
          <>
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setShowMobilePoints(false)}
              className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[2500] lg:hidden"
            />
            <motion.aside
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              ...
              className="fixed top-0 right-0 h-full w-[70%] max-w-[260px] bg-editorial-bg z-[2600] shadow-2xl flex flex-col lg:hidden border-l border-editorial-text/10"
            >
              <div className="p-4 border-b border-editorial-text/10 flex justify-between items-center bg-editorial-text text-white">
                <div className="flex items-center gap-2">
                  <MapPin size={14} className="text-brand-primary" />
                  <h3 className="text-[10px] font-bold uppercase tracking-[0.2em]">Puntos</h3>
                </div>
                <button onClick={() => setShowMobilePoints(false)} className="p-1">
                  <X size={18} />
                </button>
              </div>

              <div className="flex-1 overflow-y-auto p-1.5 flex flex-col gap-1 custom-scrollbar bg-editorial-bg/50">
                {pointsSorted.map((point) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setShowMobilePoints(false);
                    }}
                    className={`flex items-center gap-3 p-2 text-left transition-all border ${
                      (focusedPointId === point.id || selectedPoint?.id === point.id)
                        ? "bg-editorial-text text-white border-editorial-text shadow-md" 
                        : "bg-white border-editorial-text/5 active:scale-[0.98]"
                    }`}
                  >
                    <span className={`font-mono text-[9px] font-bold w-4 ${
                      (focusedPointId === point.id || selectedPoint?.id === point.id) ? "text-brand-primary" : "opacity-30"
                    }`}>
                      {point.order}
                    </span>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[10px] font-bold uppercase tracking-wider leading-tight truncate">
                        {point.title}
                      </h4>
                    </div>
                  </button>
      ...
      <footer className="absolute bottom-6 left-6 z-[1000] flex flex-col gap-4">
        <div className="flex gap-2 pointer-events-auto">
          <button 
            ...
            className="w-[42px] h-[42px] md:w-11 md:h-11 bg-editorial-bg border border-editorial-text/10 flex items-center justify-center hover:bg-brand-primary hover:text-white transition-all shadow-lg group"
            title="Sobre el proyecto"
          >
            <Info size={18} className="text-brand-primary group-hover:text-white transition-colors" />
          </button>

          <button 
            onClick={() => setShowMobilePoints(true)}
            className="lg:hidden flex items-center gap-3 bg-editorial-bg border border-editorial-text/10 px-5 py-3 hover:bg-brand-primary hover:text-white transition-all shadow-lg group"
          >
            <MapPin size={16} className="text-brand-primary group-hover:text-white transition-colors" />
            <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Puntos</span>
          </button>
        </div>
